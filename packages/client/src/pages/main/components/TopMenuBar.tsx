      ...
      <div style={st.menuBarRight}>
        ...
        >
          ...
          {unreadMentionCount > 0 && (
            <span style={st.menuBadge}>{unreadMentionCount > 9 ? '9+' : unreadMentionCount}</span>
          )}
        </button>

        <button type="button" style={{ ...st.menuBtn, ...(activePanel === 'bookmark' ? st.menuBtnActive : {}) }} onClick={() => togglePanel('bookmark')} title="북마크">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <path d="M19 21l-7-5-7 5V5a2 2 0 012-2h10a2 2 0 012 2z" />
          </svg>
        </button>

        <button type="button" style={{ ...st.menuBtn, ...(activePanel === 'friends' ? st.menuBtnActive : {}) }} onClick={() => togglePanel('friends')} title="멤버">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M22 21v-2a4 4 0 0 0-3-3.87" /><path d="M16 3.13a4 4 0 0 1 0 7.75" />
          </svg>
        </button>

        <button type="button" style={{ ...st.menuBtn, ...(activePanel === 'schedule' ? st.menuBtnActive : {}) }} onClick={() => togglePanel('schedule')} title="일정">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <rect x="3" y="4" width="18" height="18" rx="2" ry="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" />
          </svg>
        </button>

        <button type="button" style={{ ...st.menuBtn, ...(activePanel === 'ai' ? st.menuBtnActive : {}) }} onClick={() => togglePanel('ai')} title="AI 채팅">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <path d="M12 3l1.5 4.5L18 9l-4.5 1.5L12 15l-1.5-4.5L6 9l4.5-1.5L12 3z" />
          </svg>
        </button>

        <button type="button" style={{ ...st.menuBtn, ...(activePanel === 'settings' ? st.menuBtnActive : {}), position: 'relative' as const }} onClick={() => togglePanel('settings')} title="설정">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
