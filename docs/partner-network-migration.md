...
| 외부 진입 (**메신저 API**) | `121.143.3.163:3001` | UTM 대장 `3001 → 210:3001` (TCP OPEN, 앱 기동 대기) |
| 내부 호스트 | `192.168.123.210` | GW/ERP + 메신저 설치 |
| MS-SQL (조직) | 내부 `192.168.123.211:1433` | 외부로는 `9213` |
| GW DB | `CSI_GW` | 조직 sync 소스 |
| 클라이언트 기본 URL | `http://121.143.3.163:3001` | 사외용 (exe에 박힘) |

> `9210`으로 RDP(`gw_admin`) 접속 후 Docker로 API+DB를 올린다.  
> 사외 API는 **`3001`** (관리대장에 메신저용으로 이미 등록, 신규 9214 불필요).
...
- [x] **기본 API URL** — `http://121.143.3.163:3001` (사외)
...
- [ ] **사외** → UTM `3001 → 210:3001` 오픈 후 `http://121.143.3.163:3001` 확인  
...
반영됨 (`CSIN-Tech` / `com.csintech.message` / 기본 API `http://121.143.3.163:3001`):
